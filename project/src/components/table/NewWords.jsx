...
const NewWords = () => {
  ...
  const [error, setError] = useState(null); // content loaded error
  ...
  /* -- functions -- */
  const sendData = (id, action) => {
    setIsLoaded(true);
    let url;
    action !== "add" ? (url = `${id}/${action}`) : (url = `${action}`);
    let newWords = words.reverse();
    fetch(`http://itgirlschool.justmakeit.ru/api/words/${url}`, {
      method: "POST",
      headers: {
